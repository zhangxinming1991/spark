...
public class BiCubicInterpolationScale {
    ...
    public static Double[] imgScale_d(Double[] inPixelsData, int srcW, int srcH, int destW, int destH) {
        ...
        for(int row=0; row<destH; row++) {
            ...
            for(int col=0; col<destW; col++) {
                ...
                double u = srcCol - k;
             //   for(int i=0; i<4; i++) {
                    tempPixels[0] = getGrayValue(input3DData,j-1,k-1);

                    tempPixels[1] = getGrayValue(input3DData, j, k-1);

                    tempPixels[2] = getGrayValue(input3DData, j+1,k-1);

                    tempPixels[3] = getGrayValue(input3DData, j+2, k-1);

                    // update coefficients
                updateCoefficients_d(tempPixels);
                double value = getValue(t,u);
                if (value > 1.0)
                    value = 1.0;
                else if (value < 0)
                    value = 0.0;
                    ...
                    value = value;
                    outputThreeDeminsionData[row][col] = value;
              //  }
    ...
    private static double getGrayValue(Double[][] input3DData, double row, double col) {
        if(col >= srcWidth) {
            col = srcWidth - 1;
        }
        ...
        return input3DData[(int)row][(int)col];
    }
    ...
    private static void updateCoefficients_d (Double[] p) {
        a00 = p[1];
        a01 = -.5*p[1] + .5*p[1];
        a02 = p[1] - 2.5*p[1] + 2*p[1] - .5*p[1];
        a03 = -.5*p[1] + 1.5*p[1] - 1.5*p[1] + .5*p[1];
        a10 = -.5*p[0] + .5*p[2];
        a11 = .25*p[0] - .25*p[0] - .25*p[2] + .25*p[2];
        a12 = -.5*p[0] + 1.25*p[0] - p[0] + .25*p[0] + .5*p[2] - 1.25*p[2] + p[2] - .25*p[2];
        a13 = .25*p[0] - .75*p[0] + .75*p[0] - .25*p[0] - .25*p[2] + .75*p[2] - .75*p[2] + .25*p[2];
        a20 = p[0] - 2.5*p[1] + 2*p[2] - .5*p[3];
        a21 = -.5*p[0] + .5*p[0] + 1.25*p[1] - 1.25*p[1] - p[2] + p[2] + .25*p[3] - .25*p[3];
        a22 = p[0] - 2.5*p[0] + 2*p[0] - .5*p[0] - 2.5*p[1] + 6.25*p[1] - 5*p[1] + 1.25*p[1] + 2*p[2] - 5*p[2] + 4*p[2] - p[2] - .5*p[3] + 1.25*p[3] - p[3] + .25*p[3];
        a23 = -.5*p[0] + 1.5*p[0] - 1.5*p[0] + .5*p[0] + 1.25*p[1] - 3.75*p[1] + 3.75*p[1] - 1.25*p[1] - p[2] + 3*p[2] - 3*p[2] + p[2] + .25*p[3] - .75*p[3] + .75*p[3] - .25*p[3];
        a30 = -.5*p[0] + 1.5*p[1] - 1.5*p[2] + .5*p[3];
        a31 = .25*p[0] - .25*p[0] - .75*p[1] + .75*p[1] + .75*p[2] - .75*p[2] - .25*p[3] + .25*p[3];
        a32 = -.5*p[0] + 1.25*p[0] - p[0] + .25*p[0] + 1.5*p[1] - 3.75*p[1] + 3*p[1] - .75*p[1] - 1.5*p[2] + 3.75*p[2] - 3*p[2] + .75*p[2] + .5*p[3] - 1.25*p[3] + p[3] - .25*p[3];
        a33 = .25*p[0] - .75*p[0] + .75*p[0] - .25*p[0] - .75*p[1] + 2.25*p[1] - 2.25*p[1] + .75*p[1] + .75*p[2] - 2.25*p[2] + 2.25*p[2] - .75*p[2] - .25*p[3] + .75*p[3] - .75*p[3] + .25*p[3];
    }
     ...
     *
     */
    private static Integer[] convertToOneDim(int[][][] data, int imgCols, int imgRows) {
    ...
    public static Double[] convertToOneDim_d(Double[][] data, int imgCols, int imgRows) {
    ...
    public static Double[][] processOneToTwoDeminsion(Double []inPixelsData,int imgRows, int imgCols){
